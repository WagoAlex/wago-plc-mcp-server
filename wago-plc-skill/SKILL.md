...
name: wago-plc
description: |
  Use this skill whenever you need to check on, read from, write to, or
  monitor a WAGO PLC (PFC200, PFC300, CC100, CC100-IEC62443, Edge Controller,
  WP400, TP600) through the wago-plc MCP server — whether that's a human's
  plain-English request ("what's the firmware on the PLC at 192.168.1.10?",
  "turn on NTP sync", "watch the LED status every 30 seconds", "list my
  PLCs") or an autonomous agent/pipeline calling the tools programmatically
  (orchestration, monitoring daemons, scheduled jobs, multi-agent systems).
  Covers finding and describing PLCs, reading and writing parameters safely,
  invoking methods, watchlists, the MCP tool I/O contract (shapes, batching,
  error handling, retries), and — critically — recognizing that WAGO's fleet
  spans two different firmware generations (PTXdist-based and Yocto-based)
  with different parameter models, so the same plain-English request can
  need a different parameter path depending on which one you're talking to.
  Use this skill even for requests that don't say "WAGO" or "PLC" by name if
  the conversation is clearly about an industrial controller, fieldbus
  device, or factory-floor system reachable by IP address. Does not cover
  the underlying WDA REST/HTTP protocol itself (see wago-quickref/SKILL.md
  for that) — this is the MCP tool layer, for both chat and agent callers.
---
